...
@Service
public class PlaylistEventStore implements EventStore {
    ...
    for (Event event : events) {
      ...
      EventDocument eventDocument =
          EventDocument.builder()
              ...
              .aggregateType(PlaylistAggregate.class.getTypeName())
              .version(version)
      ...
      if (!eventDocument.getId().isEmpty()) {
        final String eventTopic = PLAYLIST_EVENT_TOPIC_PREFIX + event.getClass().getSimpleName();
  ...
  public List<Event> getEvents(String aggregateId) {
    ...
    if (CollectionUtils.isEmpty(eventDocuments)) {
      throw new AggregateNotFoundException("Playlist not found: id=%s".formatted(aggregateId));
    }
